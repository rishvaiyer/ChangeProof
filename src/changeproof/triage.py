...
def _sql_header(question: str) -> str:
    ...
    return f"""/* Generated by contextIsKey · DataHub context layer; review required; not executed.
...
def _validation_sql(mapped: tuple[TriageRule, ...]) -> str:
    ...
    checks = ["-- contextIsKey · DataHub context layer validation previews; review required."]
    for rule in mapped:
        checks.extend(
            [
...
def _full_validation_sql() -> str:
    return """-- contextIsKey · DataHub context layer validation checks.
